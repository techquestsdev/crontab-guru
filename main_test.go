...
// TestClipboardAvailable tests the clipboard availability detection function
func TestClipboardAvailable(t *testing.T) {
	t.Parallel()
...
// TestClipboardAvailableLinuxNoDisplay tests Linux clipboard detection without DISPLAY
func TestClipboardAvailableLinuxNoDisplay(t *testing.T) {
	t.Parallel()
	...
	defer func() {
		if originalDisplay != "" {
			t.Setenv("DISPLAY", originalDisplay)
		} else {
	...
	result := clipboardAvailable()
	if result != false {
		t.Error("Expected clipboardAvailable to return false on Linux without DISPLAY")
	}
}

// TestHandleShiftTabNavigationEdgeCases tests shift+tab navigation edge cases
func TestHandleShiftTabNavigationEdgeCases(t *testing.T) {
	t.Parallel()

	// Test with focus index at 0 (should wrap to last input)
	m := initialModel()
	m.focusIndex = 0

	cmd := m.handleShiftTabNavigation()

	if m.focusIndex != 4 {
		t.Errorf("Expected focus index to be 4 after Shift+Tab from 0, got %d", m.focusIndex)
	}

	if cmd == nil {
		t.Error("Expected a command from handleShiftTabNavigation")
	}

	// Test with focus index at middle (should move to previous)
	m.focusIndex = 2
	cmd = m.handleShiftTabNavigation()

	if m.focusIndex != 1 {
		t.Errorf("Expected focus index to be 1 after Shift+Tab from 2, got %d", m.focusIndex)
	}
...
// TestHandleBackspaceNavigationEdgeCases tests backspace navigation edge cases
func TestHandleBackspaceNavigationEdgeCases(t *testing.T) {
	t.Parallel()

	m := initialModel()

	// Test backspace on empty input at index 0 (should not navigate)
	m.focusIndex = 0
	m.inputs[0].SetValue("")

	cmd := m.handleBackspaceNavigation()

	if m.focusIndex != 0 {
		t.Errorf("Expected focus to stay at 0 when backspacing on first empty input, got %d", m.focusIndex)
	}

	if cmd != nil {
		t.Error("Expected no command from handleBackspaceNavigation on first input")
	}

	// Test backspace on empty input at index 1 (should navigate to previous)
	m.focusIndex = 1
	m.inputs[1].SetValue("")

	_ = m.handleBackspaceNavigation()

	if m.focusIndex != 0 {
		t.Errorf("Expected focus to move to 0 when backspacing on empty input at 1, got %d", m.focusIndex)
	}

	// Test backspace on non-empty input (should not navigate)
	m.focusIndex = 2
	m.inputs[2].SetValue("5")

	cmd = m.handleBackspaceNavigation()

	if m.focusIndex != 2 {
		t.Errorf("Expected focus to stay at 2 when backspacing on non-empty input, got %d", m.focusIndex)
	}

	if cmd != nil {
		t.Error("Expected no command from handleBackspaceNavigation on non-empty input")
	}
} // TestValidateLetterValueEdgeCases tests edge cases in letter validation
func TestValidateLetterValueEdgeCases(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
		...
		{"J", fieldIndexMonth, false, "single letter J in month field"},
		{"JA", fieldIndexMonth, false, "partial month abbreviation JA"},
		{"JANUARY", fieldIndexMonth, true, "full month name"},
		{"S", fieldIndexWeekday, false, "single letter S in weekday field"},
		{"SU", fieldIndexWeekday, false, "partial weekday abbreviation SU"},
		{"SUNDAY", fieldIndexWeekday, true, "full weekday name"},
		{"XYZ", fieldIndexMonth, false, "invalid letters in month field"},
		{"ABC", fieldIndexWeekday, false, "invalid letters in weekday field"},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			result := validateLetterValue(tt.value, tt.fieldIdx)
			if result != tt.expected {
				t.Errorf("validateLetterValue(%q, %d) = %v, expected %v",
					tt.value, tt.fieldIdx, result, tt.expected)
			}
		})
	}
}

// TestInitialModelWithCronDescriptorError tests initialization when cron descriptor fails
func TestInitialModelWithCronDescriptorError(t *testing.T) {
	t.Parallel()

	// This test verifies that initialModel handles cron descriptor creation errors
	// The actual error simulation would require mocking the crondesc.NewDescriptor function
	// For now, we test that initialModel returns a valid model even if there might be errors
	m := initialModel()

	if m == nil {
		t.Fatal("Expected initialModel to return a non-nil model")
	}
...
func TestHandleCopyToClipboardEdgeCases(t *testing.T) {
	...
	cmd := m.handleCopyToClipboard()

	// Should return a command for clearing the message
	if cmd == nil {
		t.Error("Expected handleCopyToClipboard to return a command")
	}
	...
	if cmd != nil {
		msg := cmd()
		if _, ok := msg.(clearCopyMessage); !ok {
			t.Errorf("Expected command to return clearCopyMessage, got %T", msg)
		}
	}

	// Check that copy message was set
	if m.copyMessage == "" {
		t.Error("Expected copy message to be set")
	}
}
...
// TestClearCopyMessageHandling tests the clearCopyMessage functionality
func TestClearCopyMessageHandling(t *testing.T) {
	t.Parallel()

	m := initialModel()
	m.copyMessage = "Test message"

	// Send clearCopyMessage
	clearMsg := clearCopyMessage{}
	newModel, cmd := m.Update(clearMsg)
	m = assertModelType(t, newModel)
	...
	if cmd != nil {
		t.Error("Expected no command from clearCopyMessage")
	}
}

// TestEdgeCaseInputValues tests various edge case input values
func TestEdgeCaseInputValues(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
		...
		{[]string{"", "", "", "", ""}, "all empty inputs"},
		{[]string{"*", "*", "*", "*", "*"}, "all wildcards"},
		{[]string{"0-59", "0-23", "1-31", "1-12", "0-6"}, "all ranges"},
		{[]string{"*/15", "*/2", "*/5", "*/3", "*/1"}, "all steps"},
		{[]string{"0,30", "9,17", "1,15", "JAN,JUL", "MON,FRI"}, "all lists"},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Create a new model instance for each subtest to avoid race conditions
			m := initialModel()
			...
			// Update description to trigger validation
			m.updateDescription()

			// Should not panic and should produce some description or error
			if m.description == "" && m.err == nil {
				t.Error("Expected either description or error to be set")
			}
		})
	}
}

// TestClipboardAvailableOtherOS tests clipboard availability on non-Linux OS
func TestClipboardAvailableOtherOS(t *testing.T) {
	t.Parallel()

	if runtime.GOOS == "linux" {
		t.Skip("This test only runs on non-Linux OS")
	}

	// On non-Linux OS, it should only check TTY availability
	result := clipboardAvailable()

	// The result depends on whether we have a TTY, but should not panic
	_ = result
}

// TestInitialModelCompleteCoverage tests more paths in initialModel
func TestInitialModelCompleteCoverage(t *testing.T) {
	t.Parallel()

	// Test that initialModel creates correct number of inputs with proper setup
	m := initialModel()

	// Verify all inputs are properly initialized
	for i, input := range m.inputs {
		if input.CharLimit != inputCharLimit {
			t.Errorf("Input %d char limit should be %d, got %d", i, inputCharLimit, input.CharLimit)
		}

		if input.Width != inputWidth {
			t.Errorf("Input %d width should be %d, got %d", i, inputWidth, input.Width)
		}
	}
	...
	// Verify other inputs are not focused
...
// TestValidateLetterValueMoreEdgeCases tests additional edge cases for validateLetterValue
func TestValidateLetterValueMoreEdgeCases(t *testing.T) {
	t.Parallel()

	// Test different field indices
	tests := []struct {
		value    string
		fieldIdx int
		expected bool
		...
		{"JAN", fieldIndexMonth, true, "valid month abbreviation"},
		{"MON", fieldIndexWeekday, true, "valid weekday abbreviation"},
		{"JAN", fieldIndexWeekday, false, "month abbreviation in weekday field"},
		{"MON", fieldIndexMonth, false, "weekday abbreviation in month field"},
		{"", fieldIndexMonth, false, "empty string in month field"},
		{"", fieldIndexWeekday, false, "empty string in weekday field"},
		{"J", 0, false, "single letter in non-month/weekday field"},
		{"JA", 1, false, "partial abbreviation in non-month/weekday field"},
	}
...
// TestHandleCopyToClipboardWithClipboardUnavailable tests clipboard when not available
func TestHandleCopyToClipboardWithClipboardUnavailable(t *testing.T) {
	t.Parallel()

	// We can't easily mock the clipboardAvailable function without changing the code,
	// but we can test the copy functionality in different environments
	m := initialModel()
	m.inputs[0].SetValue("0")
	m.inputs[1].SetValue("12")
	m.inputs[2].SetValue("*")
	m.inputs[3].SetValue("JAN")
	m.inputs[4].SetValue("MON")

	cmd := m.handleCopyToClipboard()

	// Should always return a command regardless of clipboard availability
	if cmd == nil {
		t.Error("Expected handleCopyToClipboard to return a command")
	}

	// The copy message should be set (either success, failure, or unavailable)
	if m.copyMessage == "" {
		t.Error("Expected copy message to be set")
	}
}

// TestNavigationFunctionsEdgeCases tests additional navigation edge cases
//
//nolint:paralleltest
func TestNavigationFunctionsEdgeCases(t *testing.T) {
	// Don't use t.Parallel() here as we're doing sequential operations on shared state

	// Test handleShiftTabNavigation from various positions
	positions := []int{0, 1, 2, 3, 4}
	for _, pos := range positions {
		m := initialModel() // Create fresh model for each test
		m.focusIndex = pos
		originalFocus := m.focusIndex

		cmd := m.handleShiftTabNavigation()

		expectedFocus := (originalFocus - 1 + len(m.inputs)) % len(m.inputs)
		if m.focusIndex != expectedFocus {
			t.Errorf("From position %d, expected focus to be %d, got %d",
				originalFocus, expectedFocus, m.focusIndex)
		}

		if cmd == nil {
			t.Errorf("Expected command from handleShiftTabNavigation at position %d", pos)
		}
	}

	// Test handleBackspaceNavigation with various empty/non-empty combinations
	for i := range 5 { // Use explicit count instead of len(m.inputs)
		...
		if i == 0 {
			// Should not navigate from first input
			if cmd != nil {
				t.Errorf("Expected no command from handleBackspaceNavigation at position 0")
			}
		} else {
			// Should navigate to previous input
			if m.focusIndex != i-1 {
				t.Errorf("Expected focus to move from %d to %d, got %d", i, i-1, m.focusIndex)
			}
		}
	}
}
